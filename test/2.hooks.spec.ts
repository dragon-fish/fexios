...
import { HTTPBIN_BASE_URL } from './constants'
...
describe('Fexios Hooks', () => {
  it('[HOOKS] register hooks', async () => {
    ...
    fexios.on('beforeRequest', (ctx) => {
      expect(ctx.url).to.equal(`${HTTPBIN_BASE_URL}/anything/2?foo=bar`)
      ctx.url = ctx.url.replace('2', '3').replace('bar', 'baz')
      return ctx
    })
    fexios.on('afterResponse', (ctx) => {
      expect(ctx.data).to.be.an('object')
      expect(ctx.data.url).to.equal(`${HTTPBIN_BASE_URL}/anything/3?foo=baz`)
      ctx.data.id = time
      return ctx
    })
    const { data } = await fexios.get<Todo>('/anything/1')
  ...
  it('[HOOKS] interceptors sugar', async () => {
    ...
    fexios.interceptors.request.use((ctx) => {
      expect(ctx.url).to.equal(`${HTTPBIN_BASE_URL}/anything/1`)
      ctx.url = ctx.url.replace('1', '2')
      return ctx
    })
    fexios.interceptors.response.use((ctx) => {
      expect(ctx.data).to.be.an('object')
      expect(ctx.data.url).to.equal(`${HTTPBIN_BASE_URL}/anything/2`)
      ctx.data.id = time
      return ctx
    })
